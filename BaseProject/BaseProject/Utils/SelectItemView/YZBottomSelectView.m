...
- (instancetype)initWithTitle:(NSString *)title description:(NSString *)description cancelButtonTitle:(NSString *)cancelButtonTitle destructiveButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSArray *)otherButtonTitles handler:(YZBottomSelectViewBlock)bottomSelectViewBlock {
    ...
    if (self) {
        ...
        if (title && title.length > 0) {
            ...
            titleLabel.textColor = kFF333333;
            titleLabel.textAlignment = NSTextAlignmentCenter;
            titleLabel.font = kPingFangSC_Medium_18;
            titleLabel.numberOfLines = 0;
            titleLabel.backgroundColor = kFFF5F5F5;
        ...
        if (description && description.length > 0){
            ...
            if(IS_IPHONE_X) {
                // 设置 iPhoneX 中安全区域（底部危险区域高为：34）
                titleLabel.frame = CGRectMake(Adaptation_6(16), bootomSelectViewHeight-34.0f, self.frame.size.width-Adaptation_6(32), titleHeight);
            } else {
                titleLabel.frame = CGRectMake(Adaptation_6(16), bootomSelectViewHeight, self.frame.size.width-Adaptation_6(32), titleHeight);
            }
            titleLabel.autoresizingMask = UIViewAutoresizingFlexibleWidth;
            ...
            titleLabel.font = kPingFangSC_Regular_15;
            titleLabel.numberOfLines = 0;
        ...
        if (cancelButtonTitle && cancelButtonTitle.length > 0) {
            ...
            // 此处进行判断设备是否为iPhoneX
            if(IS_IPHONE_X || SAFEAREA_INSETS_BOTTOM > 0) {
                // 设置 iPhoneX 中安全区域（底部危险区域高为：34）
        ...
        _bottomSelectView.backgroundColor = kFFF5F5F5;
        _bottomSelectView.layer.cornerRadius = Adaptation_6(8);
        _bottomSelectView.clipsToBounds = YES;
    }
...
-(instancetype)initWithCenterImage:(UIImage *)image withButtonTitle:(NSString *)title showexitBtn:(BOOL)show content:(NSString *)content{
    self = [super initWithFrame:CGRectZero];
    if (self) {
        self.frame = [UIScreen mainScreen].bounds;
        self.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
...
//        UIImageView *imageView = [UIImageView alloc]initwi
...
+ (void)showBottomSelectViewWithTitle:(NSString *)title description:(NSString *)description cancelButtonTitle:(NSString *)cancelButtonTitle destructiveButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSArray *)otherButtonTitles handler:(YZBottomSelectViewBlock)bottomSelectViewBlock {
    YZBottomSelectView *bottomSelectView = [[self alloc] initWithTitle:title description:description cancelButtonTitle:cancelButtonTitle destructiveButtonTitle:destructiveButtonTitle otherButtonTitles:otherButtonTitles handler:bottomSelectViewBlock];
    [bottomSelectView show];
}

+ (void)showCenterImage:(UIImage *)centerImage withButtonTitle:(NSString *)title showexitBtn:(BOOL)show content:(NSString *)content{
    YZBottomSelectView *bottomSelectView = [[self alloc] init];
    [bottomSelectView show];
}
